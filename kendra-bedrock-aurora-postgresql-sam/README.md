...
* Before deploying the solution, you need to set up an [Amazon Aurora PostgreSQL DB](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/Aurora.CreateInstance.html) if you don't already have one. Prior to the Aurora PostgreSQL setup, [Create and configure a VPC](https://docs.aws.amazon.com/vpc/latest/userguide/create-vpc.html) with at least two private subnets. During the database setup:
